...
export function overrideCanvasActions(
	actions: Parameters<NonNullable<TLUiOverrides['actions']>>[1],
	openPrompt: () => void
) {
	...
	actions['inline-prompt'] = {
		id: 'inline-prompt', kbd: '$i', label: 'Ask here', readonlyOk: false, onSelect: openPrompt,
	}
	return actions
}
...
export const canvasOverrides: TLUiOverrides = {
	actions(editor, actions) {
		...
		return overrideCanvasActions(actions, () => openInlinePrompt(editor))
	},
	tools(editor, tools) {
		tools.math = {
			id: 'math',
