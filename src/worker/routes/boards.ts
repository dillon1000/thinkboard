...
export async function handleBoardCreate(request: IRequest, env: Env) {
	...
	if (!title) return Response.json({ error: 'A board title is required' }, { status: 400 })
...
export async function handleBoardGet(request: IRequest, env: Env) {
	...
	if (!foundBoard) return Response.json({ error: 'Board not found' }, { status: 404 })
...
export async function handleBoardArchive(request: IRequest, env: Env) {
	...
	const access = await getBoardAccess(database, request.params.boardID, authentication.session.user.id)
	if (!access) return Response.json({ error: 'Board not found' }, { status: 404 })
	if (access.role !== 'owner') return Response.json({ error: 'Only an owner can archive a board' }, { status: 403 })
