...
export async function handleAssetUpload(request: IRequest, env: Env) {
	const objectName = getAssetObjectName(request.params.boardID, request.params.assetID)
...
export async function handleAssetDownload(request: IRequest, env: Env, ctx: ExecutionContext) {
	...
	// Uploaded files are treated as inert content even if their media type supports scripts.
