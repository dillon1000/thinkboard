...
pnpm wrangler vectorize create agentboard-documents-1024 --dimensions 1024 --metric cosine
pnpm wrangler vectorize create-metadata-index agentboard-documents-1024 --property-name boardId --type string
```
...
OCR and embedding calls use the AI Gateway named by `AI_GATEWAY_ID`. The default configuration uses Cloudflare's `default` gateway, `@cf/meta/llama-4-scout-17b-16e-instruct` for OCR, and `@cf/baai/bge-large-en-v1.5` for 1,024-dimensional embeddings.
