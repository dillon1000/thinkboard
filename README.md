...
The Smarter study mode uses Meta Muse Spark 1.1 through OpenRouter and Cloudflare AI Gateway. Set `OPENROUTER_API_KEY` in `infra/cloudflare/.dev.vars` for local development and as a Worker secret in production. The Worker returns a service-unavailable response for Smarter requests when this key is missing.
